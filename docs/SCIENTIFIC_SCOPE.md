...
## Ce que le code calcule — CONFIRMÉ
...
## Hypothèses du modèle — PROPOSITION

Les centres, largeurs, intensités relatives, fenêtres et décalages entre espèces
sont des paramètres hérités du programme interne. Les six catégories amide I sont
...
La référence DFT interne n’est pas distribuée. Un spectre de fragment fourni
par l’utilisateur n’est pas un étalon certifié du matériau. Une coïncidence de bandes doit
être interprétée comme un indice à examiner, jamais comme une preuve exclusive.

## Limites et validations restantes — À VALIDER
...
## Bibliographie héritée

**CONFIRMÉ** — Les références sont conservées dans `database.py` pour la
traçabilité du programme fourni. L'existence d'un article ne prouve pas que chaque
